...
async def execute_multi_agents(request_data: dict = None, manager=None) -> Any:
    """
    Execute multi-agent research with proper parameter handling

    Args:
        request_data: Dictionary containing query, tone, and other parameters
        manager: WebSocketManager instance

    Returns:
        Structured API response with research results
    """
    # Default fallback values
    if request_data is None:
        request_data = {}

    query = request_data.get(
        "query", "Is AI in a hype cycle?"
    )  # Still have fallback for backward compatibility
    tone = request_data.get("tone", "Objective")
    max_sections = request_data.get("max_sections", 3)
    verbose = request_data.get("verbose", False)

    # Import stream_output from gpt_researcher.actions
    from gpt_researcher.actions import stream_output

    # Set environment variables for multi-agent configuration
    os.environ["MAX_SECTIONS"] = str(max_sections)
    ...
    if websocket:
        try:
            # Convert tone string to Tone enum
            from gpt_researcher.utils.enum import Tone

            tone_enum = Tone[tone] if isinstance(tone, str) else tone
            ...
            logger.error(f"Error in execute_multi_agents: {e}")
            return JSONResponse(
                status_code=500,
                content={
                    "success": False,
